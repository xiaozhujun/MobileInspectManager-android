...
public class RFIDService extends Service {
	...
	private Handler handler = new Handler(){
		...
		private boolean authCard(int sector, String password, String cardID){
			boolean flag = false;
			byte[] cardIDBytes = CommonUtils.HexString2Bytes(cardID);
			int cardIDLen = cardIDBytes.length;
			byte[] passwordBytes = CommonUtils.HexString2Bytes(password);
			Log.i("duibi", "0"+";"+sector+";"+passwordBytes.length+";"+cardIDLen+";"+cardIDBytes.length);
		...
		private String readCard(String block){
			String data = null;
			byte[] dataBytes = null;
			byte[] blockBytes = CommonUtils.HexString2Bytes(block);
		...
		private boolean writeCard(String block, String data){
			boolean flag = false;
			byte[] dataBytes = CommonUtils.HexString2Bytes(data);
//			byte[] blocks = Tools.HexString2Bytes(block);
	...
	public void onCreate() {
		...
		filter.addAction("org.whut.service.RFIDService");
					...
					if(cardID14443A != null ){
						...
						if(cmdManager.deInit_14443A()){
							...
							if(cmdManager.initMifare14443A()){
								cardID = CommonUtils.Bytes2HexString(cardID14443A, cardID14443A.length);
